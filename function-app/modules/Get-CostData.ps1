...
function Get-CostData {
    ...
    # The orchestrator pre-loads cost rows from the hub export and passes
    # them in. Reading the export is far faster and avoids Cost Management
    # API throttling. The live API stays available as the fallback below.
    if ($HubData -and @($HubData).Count -gt 0) {
        Write-Host "  Using FinOps Hub export data for cost ($(@($HubData).Count) rows)..." -ForegroundColor Green
        try {
            $hubCost = ConvertTo-CostDataFromHub -HubData $HubData
            if ($hubCost -and $hubCost.Count -gt 0) { return $hubCost }
            Write-Host "  Hub export had no cost rows — falling back to Cost Management API" -ForegroundColor Yellow
        }
        catch {
            Write-Warning "  Hub cost conversion failed: $($_.Exception.Message) — falling back to API"
        }
    }
