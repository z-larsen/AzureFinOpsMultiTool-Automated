...
function Get-ResourceCosts {
    ...
    # When the orchestrator supplies hub export rows, aggregate per-resource
    # spend straight from the export (fast, no per-subscription Cost
    # Management calls). The live API path below is the fallback.
    if ($HubData -and @($HubData).Count -gt 0) {
        Write-Host "  Using FinOps Hub export data for resource costs ($(@($HubData).Count) rows)..." -ForegroundColor Green
        try {
            $hubRows = ConvertTo-ResourceCostsFromHub -HubData $HubData
            if ($hubRows -and @($hubRows).Count -gt 0) { return @($hubRows) }
            Write-Host "  Hub export had no resource rows — falling back to Cost Management API" -ForegroundColor Yellow
        }
        catch {
            Write-Warning "  Hub resource-cost conversion failed: $($_.Exception.Message) — falling back to API"
        }
    }
