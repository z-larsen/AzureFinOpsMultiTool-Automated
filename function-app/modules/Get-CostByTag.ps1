...
function Get-CostByTag {
    ...
    # -- Export-first: use FinOps Hub data when provided ----------------
    # Allocate cost across tag values straight from the export rows instead
    # of issuing per-tag Cost Management queries. Falls through to the live
    # API path below if the hub conversion yields nothing.
    if ($HubData -and @($HubData).Count -gt 0) {
        Write-Host "  Using FinOps Hub export data for cost-by-tag ($(@($HubData).Count) rows)..." -ForegroundColor Green
        try {
            $hubByTag = ConvertTo-CostByTagFromHub -HubData $HubData -ExistingTags $ExistingTags
            if ($hubByTag -and -not $hubByTag.NoTagsFound) { return $hubByTag }
            Write-Host "  Hub export produced no tagged cost — falling back to Cost Management API" -ForegroundColor Yellow
        }
        catch {
            Write-Warning "  Hub cost-by-tag conversion failed: $($_.Exception.Message) — falling back to API"
        }
    }
